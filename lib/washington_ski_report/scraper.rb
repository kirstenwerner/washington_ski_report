class WashingtonSkiReport::Scraper
  ...
  def scrape_snow
    ...
    url_array.map do |url| 
      ...
      new_snow_detail = doc_url.css('table').css('td')[15].text.gsub("   ", " - ").split("\"").map {|str| str + "\""}.join(", ")
      ...
      runs_open = doc_url.css('table').css('td')[5].text.gsub("\n", "").gsub('to', 't o').gsub('of', 'of ').gsub(/[ ]{2,}/, " ")
      lifts_open = doc_url.css('table').css('td')[6].text.gsub("\n", "").gsub('to', 't o').gsub('of', 'of ').gsub(/[ ]{2,}/, " ")
